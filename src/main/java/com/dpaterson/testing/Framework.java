...
public class Framework {
   ...
   */
  protected void collectClasses(List<CUTChromosome> classes, File folder) {
   ...
   *          - a list of {@link CUTChromosome} objects, which may be empty but not {@code null}
   * @param extraClasses
   *          - a list of Classes that do not contain any test cases but are still required for instrumentation. See {@link #addTestClass(List, List, File)} for a detailed desription of why this is
   *          necessary
   * @param folder
   *          - either a directory or a single file to be added to the classes list
   ...
   * The static inner class TestX contains no test cases but is still instantiated from the testing class, meaning when we come to execute JUnit, if TestX is using a different class loader from either
   * its superclass X or even from TestXYZ, there will be an error while running JUnit.
   *
   * @param classes
   ...
   *          - the current file for inspection
   */
  protected void addClass(List<CUTChromosome> classes, File file) {
    if (!file.getName().endsWith(".class")) {
      return;
    }
    PrintStream sysOut = System.out;
  ...
  public void run() {
    ...
    if (Properties.PRIORITISE) {
      logger.info("Finished parsing source folder. Found " + sut.getClassesUnderTest().size() + " source classes and "
          + testSuite.size() + " test cases");
      algorithm.start();
    }
